...
public class HomeWork {
    public static void main(String[] args) {
        ...
        {
            System.out.println("\nTask 3\n");
            try (FileInputStream fisText = new FileInputStream("Lesson15/src/main/java/by/tms/task3/text.txt");
                 FileInputStream fisCheck = new FileInputStream("Lesson15/src/main/java/by/tms/task3/blackList.txt")) {
                ...
                for (String word : words) {
                    for (String sentence : sentences) {
                        int index = (sentence.toLowerCase()).indexOf(word.trim().toLowerCase());
                        if (index > 0) {
        ...
        {
            System.out.println("\nTask 4\n");
            try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("Lesson15/src/main/java/by/tms/task4/resources/car.dat"));
                 ObjectInputStream ois = new ObjectInputStream(new FileInputStream("Lesson15/src/main/java/by/tms/task4/resources/car.dat"))) {
                Car car = new Car("BMW", new Engine("3.0", 4), new GasTank(40, "Benzin"), 230, 25000);
                oos.writeObject(car);

                Car bmw = (Car) ois.readObject();
                System.out.printf("Model: %s\nEngine type: %s\nGas tank:\nTotal volume: %.2f\nFuel type: %s\nMax speed: %.2f\nPrice: %.2f",
                        bmw.getModel(), bmw.getEngine().getEngineType(), bmw.getGasTank().getTotalVolume(), bmw.getGasTank().getFuelType(),
